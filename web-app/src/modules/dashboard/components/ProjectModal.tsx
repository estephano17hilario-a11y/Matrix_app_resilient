    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (initialData) {
                ...
                setShowAdvancedPomo(!!(initialData.focusRoutine && initialData.focusRoutine.length > 0));
            } else {
                setTitle('');
                setDesc('');
                            ...
                            <AnimatePresence initial={false}>
                                {expandedBlock === 3 && (
                                    ...
                                    >
                                        ...
                                        {/* Advanced Pomodoro Toggle Button */}
                                        <div className="flex justify-center my-1">
                                            <button 
                                                type="button"
                                                onClick={() => setShowAdvancedPomo(!showAdvancedPomo)}
                                                className={cn(
                                                    "px-3.5 py-1.5 rounded-xl border text-[9px] font-black uppercase tracking-widest transition-all duration-200 active:scale-95 flex items-center gap-1.5 shadow-md",
                                                    showAdvancedPomo 
                                                        ? "bg-cyan-500/10 border-cyan-500/30 text-cyan-400 shadow-cyan-500/5" 
                                                        : "bg-white/5 border-white/10 text-slate-400 hover:bg-white/10 hover:border-white/20"
                                                )}
                                            >
                                                <span>⚡ {showAdvancedPomo ? 'Ocultar Pomodoro Avanzado' : 'Pomodoro Avanzado'}</span>
                                                <span className="text-[7px] opacity-60">{showAdvancedPomo ? '▲' : '▼'}</span>
                                            </button>
                                        </div>
