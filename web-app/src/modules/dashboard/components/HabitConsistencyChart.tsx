...
export const HabitConsistencyChart: React.FC<HabitConsistencyChartProps> = React.memo(({ habits, onOpenStreak, isActive = true, isPro, onOpenPro, weekStartDay = 1, initialTimeframe = 'WEEK' }) => {
    ...
    return (
        <div data-tour="habit-chart" className="w-full max-w-[440px] mx-auto bg-[#0a0a0a]/70 rounded-[32px] p-4 border border-white/5 shadow-md overflow-hidden relative group">
            ...
            {/* --- COMPACT CHART AREA --- */}
            <div className="h-32 flex items-end justify-between gap-1 relative mb-0 pt-3 pl-8">
                {/* Horizontal Guidelines */}
                <div className={cn("absolute left-8 right-0 top-3 flex flex-col justify-between pointer-events-none", showTicks ? "bottom-9" : "bottom-5")}>
                    {[100, 50, 0].map((val) => (
                        <div key={val} className="w-full h-px bg-white/5 relative">
                            <span className="absolute top-1/2 -translate-y-1/2 -left-8 w-8 text-right pr-1.5 text-[9px] text-zinc-400 font-mono">{val}%</span>
                        </div>
                    ))}
                </div>
                ...
                {chartData.map((data, i) => (
                    ...
                    >
                        ...
                        {/* Label & Ticks */}
                        <div className={cn("flex flex-col items-center shrink-0 w-full justify-end", showTicks ? "mt-2 h-7" : "mt-1 h-4")}>
                            {showTicks && (
