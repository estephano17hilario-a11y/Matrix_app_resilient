...
export const Dock = React.memo(({ currentView, onChangeView, onOpenModal, isOpen, onToggle, isHidden }: { currentView: string, onChangeView: (v: string) => void, onOpenModal: (m: string) => void, isOpen: boolean, onToggle: (open: boolean) => void, isHidden: boolean }) => {
    ...
    return (
        <div className={`fixed bottom-6 left-0 right-0 z-50 flex justify-center pointer-events-none transition-transform duration-1000 cubic-bezier(0.32,0.72,0,1) ${isHidden ? 'translate-y-[200%]' : 'translate-y-0'}`}>
           <div className={`pointer-events-auto relative overflow-hidden box-border w-[88vw] max-w-[330px] transition-[height,border-radius,background-color,box-shadow] duration-500 cubic-bezier(0.32,0.72,0,1) ${isOpen ? 'h-[340px] rounded-[32px] glass-panel' : 'h-[70px] rounded-[34px] glass-panel'}`}>
            <div className="relative w-full h-full z-10">
                <div className={`absolute bottom-[80px] left-0 right-0 px-5 grid grid-cols-2 gap-2 transition-all duration-300 ease-out ${isOpen ? 'opacity-100 translate-y-0 delay-75' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                 ...
                 <div className="absolute bottom-0 left-0 right-0 h-[70px] flex items-center justify-between px-8 z-20">
                  ...
                  <button onClick={() => onToggle(!isOpen)} className={`glass-button z-20 flex items-center justify-center transition-all duration-500 cubic-bezier(0.19, 1, 0.22, 1) transform-gpu backface-hidden ${isOpen ? 'w-16 h-12 translate-y-[2px]' : 'w-14 h-14 hover:scale-105'}`}>
