            ...
            <div className="relative z-10 w-full max-w-[360px]">
                ...
                >
                    ...
                    <div className="flex justify-between items-center mb-6 px-1 shrink-0">
                        <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl flex items-center justify-center shadow-lg transition-colors duration-500" style={{ background: attrId ? activeColor : 'linear-gradient(135deg, #06b6d4, #2563eb)' }}><InfinityIcon size={20} className="text-white" /></div><div><h2 className="text-xl font-black text-white tracking-tight leading-none">{t('modals.habit.title')}</h2><span className="text-[10px] text-white/50 font-bold uppercase tracking-wider">{t('modals.habit.subtitle')}</span></div></div>
                    ...
                    <div className="overflow-y-auto no-scrollbar pb-4 space-y-3">
                        ...
                        <div className="bg-white/5 rounded-[1.5rem] border border-white/5 p-4"><input type="text" value={desc} onChange={(e) => setDesc(e.target.value)} placeholder={t('modals.habit.descPlaceholder')} className="w-full bg-transparent text-sm font-medium text-slate-300 placeholder:text-white/20 outline-none" /></div>
                        <div className="bg-white/5 rounded-[1.5rem] border border-white/5 p-1 overflow-hidden transition-all duration-300">
                             <div className="flex p-1 gap-1">{['DAILY', 'WEEKLY', 'MONTHLY'].map(f => (<button key={f} onClick={() => setFreq(f)} className={`flex-1 py-3 rounded-[1.2rem] text-[10px] font-black tracking-wide transition-all ${freq === f ? 'bg-white text-black shadow-lg' : 'text-slate-500 hover:text-white'}`}>{t(`modals.habit.frequencies.${f}`)}</button>))}</div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div className="bg-white/5 rounded-[1.5rem] border border-white/5 p-4 flex flex-col justify-center relative overflow-hidden group"><div className="flex items-center gap-2 mb-1"><Clock size={16} className="text-purple-400" /><span className="text-[10px] font-bold text-slate-400 uppercase">{t('modals.habit.alert')}</span></div><input type="time" value={reminder} onChange={(e) => setReminder(e.target.value)} className="bg-transparent text-2xl font-black text-white outline-none w-full z-10 relative" />{!reminder && <span className="absolute left-4 bottom-4 text-2xl font-black text-white/10 pointer-events-none">OFF</span>}</div>
