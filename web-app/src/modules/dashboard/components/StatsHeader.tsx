...
export const StatsHeader = React.memo(({ level, xp, nextXp, health, streak, isHidden, showProfile, hideAvatar, onShowStore, onShowPro, onShowSettings, onToggleProfile, displayName, email, currentView }: StatsHeaderProps) => {
  ...
  return (
    <header className={`flex justify-between items-center z-50 relative ${
        ...
        {/* RIGHT ACTIONS */}
        <div className={`flex items-center gap-3 ${!isHidden ? 'pointer-events-auto' : ''}`}>
             {/* PRO Button */}
            ...
            <button 
                onClick={onShowStore} 
                className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all active:scale-95 group ${
                    currentView === 'STORE'
